"""
https://www.acmicpc.net/problem/7562
문제
체스판 위에 한 나이트가 놓여져 있다. 나이트가 한 번에 이동할 수 있는 칸은 아래 그림에 나와있다. 나이트가 이동하려고 하는 칸이 주어진다. 나이트는 몇 번 움직이면 이 칸으로 이동할 수 있을까?


입력
입력의 첫째 줄에는 테스트 케이스의 개수가 주어진다.

각 테스트 케이스는 세 줄로 이루어져 있다. 첫째 줄에는 체스판의 한 변의 길이 l(4 ≤ l ≤ 300)이 주어진다.
체스판의 크기는 l × l이다. 체스판의 각 칸은 두 수의 쌍 {0, ..., l-1} × {0, ..., l-1}로 나타낼 수 있다. '
둘째 줄과 셋째 줄에는 나이트가 현재 있는 칸, 나이트가 이동하려고 하는 칸이 주어진다.

출력
각 테스트 케이스마다 나이트가 최소 몇 번만에 이동할 수 있는지 출력한다.

예제 입력 1
3
8
0 0
7 0
100
0 0
30 50
10
1 1
1 1
예제 출력 1
5
28
0
"""

import sys
from collections import deque

input = sys.stdin.readline
sys.setrecursionlimit(10**6)

n = int(input())

directions = [(2, 1), (2, -1), (-2, 1), (-2, -1), (1, 2), (1, -2), (-1, 2), (-1, -2)]


def bfs(start, target, l):
    dq = deque()
    visited = [[False] * l for _ in range(l)]
    dq.append((start[0], start[1], 0))  # (x, y, 시간)
    visited[start[0]][start[1]] = True  # 시작 위치 방문 처리
    while dq:
        x, y, time = dq.popleft()  # 현재 위치와 시간을 꺼냄
        if (x, y) == target:  # 현재 위치가 목표 위치와 같으면
            return time  # 시간을 반환
        # 다음 위치를 계산
        for dx, dy in directions:
            nx, ny = x + dx, y + dy
            if (
                0 <= nx < l and 0 <= ny < l and not visited[nx][ny]
            ):  # 범위 내에 있고 방문하지 않은 위치
                visited[nx][ny] = True  # 방문 처리
                dq.append((nx, ny, time + 1))
    return -1  # 도달할 수 없는 경우


for _ in range(n):
    l = int(input())
    start = tuple(map(int, input().split()))
    target = tuple(map(int, input().split()))
    result = bfs(start, target, l)
    print(result)
